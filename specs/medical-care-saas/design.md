...
机构控制台在产品层使用“应用中心”模型。业务应用具有稳定编码、显示名称、版本、套餐权限、机构启用状态、平台维护状态、只读降级状态和依赖清单。登录、租户隔离、权限、审计、文件安全及数据生命周期为不可停用的基础模块；考勤、审批、表单、服务履约、合同、核销、宣传和食品追溯等业务应用可按机构启停。

停用业务应用不执行删除或数据库重置：停止该应用的新建与写入，保留历史查询、必要导出和审计，并在重新启用后继续使用原数据。前端为每个应用建立独立路由、加载边界和错误边界；后端按模块返回健康或维护状态，使单个模块故障不会把整个控制台变成不可用页面。存在依赖的模块在变更状态前执行依赖校验并展示影响范围。
...
- 前、中、后三阶段分别形成 `stage_record`；退回修改写入 `record_version`，旧版本不删除。
