...
Page({
    ...
    async load() {
        ...
        try {
            this.setData({
                records: await (0, api_1.request)("/staff/food-trace-records"),
                loading: false,
            });
        }
        catch (error) {
    ...
    async chooseImage() {
        try {
            ...
            for (const file of result.tempFiles) {
                ...
                uploaded.push(await (0, api_1.request)("/staff/media", "POST", {
                    ...payload,
                    mediaType: "IMAGE",
    ...
    async uploadAudio(result) {
        try {
            ...
            const voice = await (0, api_1.request)("/staff/media", "POST", {
                ...payload,
                mediaType: "AUDIO",
    ...
    editReturned(event) {
        const record = this.data.records.find((item) => item.id === String(event.currentTarget.dataset.id));
    ...
    async submit() {
        ...
        try {
            ...
            await (0, api_1.request)("/staff/food-trace-records", "POST", {
                ...
                evidenceIds: this.data.images.map((item) => item.id),
